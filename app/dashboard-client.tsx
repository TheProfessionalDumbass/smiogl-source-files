...
type LessonDetail={lesson:Lesson&{content:{summary:string;steps:string[];formula:string}};question:{id:string;prompt:string;type:string;choices:string[]|null;hint1:string;hint2:string}|null;verification:boolean};
...
export default function DashboardClient({authenticatedUser}:{authenticatedUser:AuthUser}){
  ...
  return <main className="shell">
    ...
    <section className="content">
      <header><div className="mobilelogo"><BrandLogo /></div><div className="wallet"><span>🔥 <b>{data.inventory.streak}</b><small>day streak</small></span><span>◉ <b>{data.inventory.coins}</b><small>coins</small></span><span>💡 <b>{data.inventory.hints}</b><small>hints</small></span><button aria-label="Notifications">♢<i/></button></div></header>
      <div className="page">
...
function Rooms({data,refresh,openRoom,notify}:{data:Bootstrap;refresh:()=>Promise<void>;openRoom:(c:string)=>void;notify:(m:string)=>void}){
  ...
  <div className="notice"><b>Fair assessment, careful interpretation</b><p>The system can record visibility loss, focus loss, fullscreen exits, and connection loss. These are contextual review signals—not proof of dishonesty and never identification of a specific external app or website.</p></div></section>;
}
...
function LessonModal({detail,data,close,reload,notify}:{detail:LessonDetail;data:Bootstrap;close:()=>void;reload:()=>Promise<void>;notify:(m:string)=>void}){
  ...
  {detail.lesson.state==='review_required'?<div className="reviewgate"><h3>Full review required</h3><p>After three failures, the level was locked and this lesson was resent. Read it completely, then reopen practice—or use a revive token.</p><div><button onClick={()=>void review()}>I completed the review</button><button disabled={!data.inventory.revives} onClick={()=>void revive()}>Use revive ({data.inventory.revives})</button></div></div>:detail.question&&<div className="question"><small>{detail.verification?'Verification problem':'Practice problem'}</small><h2>{detail.question.prompt}</h2>{detail.question.choices?<div className="answers">{detail.question.choices.map(c=><button className={answer===c?'chosen':''} onClick={()=>setAnswer(c)} key={c}>{c}</button>)}</div>:<MathAnswerField value={answerLatex} onChange={(latex,plainText)=>{setAnswerLatex(latex);setAnswer(plainText)}}/>}{feedback&&<div className={feedback.correct?'feedback correct':'feedback'}>{feedback.message||feedback.hint||feedback.specialHint}{feedback.explanation&&<p>{feedback.explanation}</p>}</div>}<div className="modalactions"><button className="outline" disabled={busy||!data.inventory.hints} onClick={()=>void submit(true)}>💡 Special hint ({data.inventory.hints})</button><button className="primary" disabled={busy||!answer} onClick={()=>void submit(false)}>Check answer</button></div></div>}</section></div>;
}
